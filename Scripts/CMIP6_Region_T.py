# Change in annual average temperature in CMIP6
...
## note that we are using models that have tasmax
## because we want to match these T change values with tasmax change values
...
for m in models:
    ...
    tas_ds_hist = xr.open_mfdataset(loc_cmip6+"historical/tas_Amon/"+"tas_Amon"+"_"+model_hist+"*.nc",
                                concat_dim="time")
    ...
    tas_ds_ssp = xr.open_mfdataset(loc_cmip6+ssp+"/tas_Amon/"+"tas_Amon"+"_"+model_ssp+"*.nc",
                                concat_dim="time")
    ...
    tas_hist_final = xr.concat([tas_hist,tas_ssp],dim="time")
    del([tas_hist,tas_ssp])

    print("reading historical-nat data",flush=True)
    tas_ds_histn = xr.open_mfdataset(loc_cmip6+"historical-nat/tas_Amon/"+"tas_Amon"+"_"+model_histn+"*.nc",
                                concat_dim="time")
    tm_histn = tas_ds_histn.coords["time"].load()
    tm_histn_ind = (tm_histn.dt.year>=y1)&(tm_histn.dt.year<=y2)
    tas_histnat_final = tas_ds_histn.tas[tm_histn_ind,:,:].load()
    if tas_histnat_final.max()>200:
        tas_histnat_final = tas_histnat_final-273.15

    print("processing and interpolating",flush=True)
    # standardize calendar
    cal = pd.date_range(start=str(y1)+"-01-01",end=str(y2)+"-12-31",freq="MS")
    tas_hist_final.coords["time"] = cal
    tas_histnat_final.coords["time"] = cal

    # regrid
    if (("latitude" in tas_hist_final.coords)&("longitude" in tas_hist_final.coords)):
        tas_hist_final = tas_hist_final.rename({"latitude":"lat","longitude":"lon"})
    if (("latitude" in tas_histnat_final.coords)&("longitude" in tas_histnat_final.coords)):
        tas_histnat_final = tas_histnat_final.rename({"latitude":"lat","longitude":"lon"})
    tas_hist_interp = tas_hist_final.interp(lat=lat_new,lon=lon_new,method="linear")
    del(tas_hist_final)
    tas_histnat_interp = tas_histnat_final.interp(lat=lat_new,lon=lon_new,method="linear")
    del(tas_histnat_final)

    print("calculating annual mean",flush=True)
    ...
    tas_yr_hist = monthly_to_yearly_mean(tas_hist_interp)
    tas_yr_histnat = monthly_to_yearly_mean(tas_histnat_interp)
    tas_yr_hist_incorrect = tas_hist_interp.resample(time="YS").mean(dim="time")
    if m==models[0]:
        print(tas_yr_hist)
        print(tas_yr_hist_incorrect)
    #tas_yr_histnat = tas_histnat_interp.resample(time="YS").mean(dim="time")
    del([tas_hist_interp,tas_histnat_interp,tas_yr_hist_incorrect])
    ...
    tas_region_hist = calc_region_average(tas_yr_hist)
    tas_region_histnat = calc_region_average(tas_yr_histnat)

    #print(tas_region_hist[["AUS" in x for x in tas_region_hist.region.values],:].sel(time="1993-01-01"))

    print("combining into dataframe and writing out",flush=True)

    hist_ds = xr.Dataset({"t":(["region","time"],tas_region_hist)},
                          coords={"region":(["region"],tas_region_hist.coords["region"]),
                                  "time":(["time"],tas_region_hist.coords["time"])})
    ...
    hist_ds.attrs["variable_description"] = "Annual average temperature at adm1 region level"
    hist_ds.attrs["created_from"] = os.getcwd()+"/CMIP6_Region_T.py"

    fname_out = loc_out+m+"_historical-"+ssp+"_t_region_annual_"+str(y1)+"-"+str(y2)+".nc"
    hist_ds.to_netcdf(fname_out,mode="w")
    print(fname_out,flush=True)



    histnat_ds = xr.Dataset({"t":(["region","time"],tas_region_histnat)},
                          coords={"region":(["region"],tas_region_histnat.coords["region"]),
                                  "time":(["time"],tas_region_histnat.coords["time"])})
    ...
    histnat_ds.attrs["variable_description"] = "Annual average temperature at adm1 region level"
    histnat_ds.attrs["created_from"] = os.getcwd()+"/CMIP6_Region_T.py"

    fname_out = loc_out+m+"_historical-nat_t_region_annual_"+str(y1)+"-"+str(y2)+".nc"
